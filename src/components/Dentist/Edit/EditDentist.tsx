...
interface EditDentistPageProps {}

const EditDentistPage: React.FC<EditDentistPageProps> = () => {
  const [clinicDentals, setClinicDentals] = useState<ClinicDetail[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [form] = useForm();
  const navigate = useNavigate();
  const dentistToEdit = useLocation().state as LocationData;
  ...
  return (
    <>
      <Typography.Title level={1}>Chỉnh sửa thông tin</Typography.Title>
      <Form
        form={form}
        labelCol={{ span: 6 }}
        wrapperCol={{ span: 218 }}
        initialValues={dentistToEdit.data}
        onFinish={(value) => {
          console.log(value);
        }}
        style={{ width: "100%" }}>
